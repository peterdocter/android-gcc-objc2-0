...
static const _Unwind_Exception_Class __objc_exception_class = ((((((((_Unwind_Exception_Class) 'G'
...
get_ttype_entry (struct lsda_header_info *info, _Unwind_Word i)
{
	...
	if (ptr)
		return objc_getClass ((const char *) ptr);
	else
		return 0;
}
#else
...
isKindOf (id value, Class target)
{
	...
	for (c = value->isa; c; c = class_get_super_class (c))
		if (c == target)
			return 1;
	return 0;
}
